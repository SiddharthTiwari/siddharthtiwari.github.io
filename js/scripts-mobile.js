...
// var TweenLite = window.TL = require("lib/tweenlite/TweenLite");
// var EasePack = require("lib/tweenlite/easing/EasePack");

var Stage = require("lib/anonymous/core/Stage");

var Config = require("particles/Config");

var Menu = require("menu/Menu");
var Lines = require("fx/Lines");
...
MainParticles.prototype = 
{
	init: function()
	{
		...
		// particles
		// this._particles = new Particles(this._content);
		...
		this._menu = new Menu();
		this._menu.$
			.on("change", $.proxy(this._onChangeEntry, this));

		// stats
		// this._stats = new Stats();
		// this._stats.domElement.style.top = '0px';
		// this._content.appendChild(this._stats.domElement);
	},
	...
	_onChangeEntry: function(e, img)
	{
		// this._particles.setTexture(img);
	}
};
...
},{"fx/Lines":2,"lib/anonymous/core/Stage":3,"lib/anonymous/events/Event":4,"lib/anonymous/utils/RequestAnimationFrame":6,"lib/zepto/Ease":7,"lib/zepto/zepto":8,"menu/Menu":9,"particles/Config":10}],2:[function(require,module,exports){
"use strict";
 ...
 * @constructor
 */
var Lines = module.exports = function()
{
	this.init();
};
...
Lines.prototype =
{
	init: function()
	{
		...
		for (var i = 0, l = this._$lines.length; i < l; ++i)
		{
			var line = this._$lines[i];
			line.innerHTML = '<span class="line__value">' + line.innerHTML + '</span><span class="line__bg"></span>';

			$(line).children(".line__bg").animate({"translate": "-100%,0"}, 0);

			this._lines[i] = $(line);
		}

		$(".mask").animate({"translate": "100%,0"}, 1100, Easing.inOutQuart, null, 200);
		$(".background").animate({"opacity": 1, "translate": "0,0"}, 1300, Easing.inOutQuart, null, 200);
		setTimeout($.proxy(this.show, this), 800);
	},
	...
	show: function()
	{
		for (var i = 0, l = this._lines.length; i < l; ++i)
		{
			...
			setTimeout($.proxy(function(line)
			{
				this._showLine(line);
			}, this, line), i * 150);
		}
	...
	_showLine: function(line)
	{
		line.css("visibility", "visible");
		var $line = $(line);
		var $bg = $line.children(".line__bg");
		var $value = $line.children(".line__value");

		$bg.animate({"translate": "0,0"}, 300, Easing.inQuart, function()
		{
			$value[0].style.visibility = "visible";
			$(this).animate({"translate": "101%,0"}, 500, Easing.outExpo, function()
			{
				// $(this).css("will-change", "none");
			});
		});
	}
};
...
Menu.prototype = 
{
	init: function()
	{
		...
		this._$links = $("a[data-img]");//.not('a[disabled="disabled"]');

		for (var i = 0, l = this._$links.length; i < l; ++i)
		{
			var link = this._$links[i];
			link.innerHTML = '<span class="link__value">' + link.innerHTML + '</span><span class="link__bg"></span>';

			$(link).children(".link__bg").animate({"translate": "-101%,0"}, 0);

			// this._lines[i] = $(link);
		}

		this._$links
			.on(MouseEvent.ENTER, $.proxy(this._onEnterEntry, this))
			.on(MouseEvent.LEAVE, $.proxy(this._onLeaveEntry, this));

		Stage.$window.on(MouseEvent.CLICK, $.proxy(this._onClickWindow, this));
	...
	_onEnterEntry: function(e)
	{
		...
		var filename = $(target).attr("data-img");

		if(filename !== "")
		{
			this._loadImg("/img/" + filename);

			$bg = $target.children(".link__bg");
			$value = $target.children(".link__value");

			$value.animate({"color": "white"}, 300, Easing.outQuart);
			$bg.animate({"translate": "0,0"}, 300, Easing.outExpo);
		}
	},
	...
	_loadImg: function(src)
	{
		...
		else
		{
			...
			{
				that._onImageLoaded(img);
			});
		}
	},

	_onLeaveEntry: function(e)
	{
		this._active = false;
	...
	_onImageLoaded: function(img)
	{
		this.$.trigger("change", [img]);
	},
	...
	_clearActive: function(click)
	{
		var active = this._$links.filter(".active");
		if(active.length)
		{
			var $bg = active.children(".link__bg");
			var $value = active.children(".link__value");

			if(click)
			{
				$value.animate({"color": "inherit"}, 500, Easing.inOutExpo);
				$bg.animate({"translate": "-100%,0"}, 500, Easing.inOutExpo);
			}
			else
			{
				$value.animate({"color": "inherit"}, 300, Easing.outQuart);
				$bg.animate({"translate": "-100%,0"}, 300, Easing.outExpo);
			}	
...
},{"lib/anonymous/events/MouseEvent":5}],10:[function(require,module,exports){
"use strict";
